...
function countVowels(str) {
  ...
  const vowelMatchs = str.match(Vowels);
  return vowelMatchs ? vowelMatchs.length : 0;
}
...
function isPalindrome(str) {
  const defaultStr = str.toLowerCase().replace(/[^a-z]/g, '');
  const reversedStr = defaultStr.split('').reverse().join('');
  return reversedStr === defaultStr;
}
...
function findLongestWord(sentence) {
  ...
  let theLongestWordOfSentence = '';
  wordsOfSentence.forEach((word) => {
    if (word.length > theLongestWordOfSentence.length) {
      theLongestWordOfSentence = word;
    }
  });
  return theLongestWordOfSentence;
}
...
function invertCase(str) {
  ...
  letters.forEach((letter) => {
    ...
    } else invertedStr += letter.toLowerCase();
  });
  return invertedStr;
}
...
function extractEmails(str) {
  const splitedMails = str.split(';');
  const extractedMails = [];
  splitedMails.forEach((mail) => {
    extractedMails.push(mail);
  });
...
function encodeToRot13(str) {
  const defaultStr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const encodedStr = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let encodeWord = '';
  for (let i = 0; i < str.length; i += 1) {
    const letter = str.charAt(i);
    const index = defaultStr.indexOf(letter);
    if (index !== -1) {
      encodeWord += encodedStr.charAt(index);
    } else {
      encodeWord += letter;
    }
  }
